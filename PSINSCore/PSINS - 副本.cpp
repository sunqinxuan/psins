...
Date: 17/02/2015, 19/07/2017, 11/12/2018, 27/12/2019, 12/12/2020, 22/11/2021, 17/10/2022, 25/03/2023
*/
...
{
//	std::cout<<"CVect::Set2"<<std::endl;
	psinsassert(rc<=MMD);
//	std::cout<<"CVect::Set2 1"<<std::endl;
	va_list vl;
//	std::cout<<"CVect::Set2 2"<<std::endl;
	va_start(vl, f);
//	std::cout<<"CVect::Set2 3"<<std::endl;
	for(int i=0; i<rc; i++)
	{ if(f>2*INF) break;  dd[i] = f*f;  f = va_arg(vl, double);	}
//	std::cout<<"CVect::Set2 4"<<std::endl;
	va_end(vl);
//	std::cout<<"CVect::Set2 5"<<std::endl;
}
...
{
//	std::cout<<"CKalman::Init"<<std::endl;
	kftk = 0.0;
	nq = nq0; nr = nr0;
//	std::cout<<"nq="<<nq<<std::endl;
	Ft = Pk = CMat(nq,nq,0.0);
//	std::cout<<"CKalman::Init 1"<<std::endl;
...
{
//	std::cout<<"CSINSTDKF::CSINSTDKF"<<std::endl;
//	std::cout<<"nq0="<<nq0<<",nr0="<<nr0<<std::endl;
	CKalman::Init(nq0, nr0);
	Kkp2y = Kkv2y = 1.0;
//	std::cout<<"CSINSTDKF::CSINSTDKF return"<<std::endl;
}
...
//			for(pFk=Fk.dd,pFt=Ft.dd,pEnd=&Fk.dd[cststt*Fk.clm]; pFk<pEnd; pFk++,pFt++)  *pFk=*pFt*tdts;
			for(int i=0;i<cststt*Fk.clm;i++)
			{
				Fk.dd[i]=Ft.dd[i]*tdts;
			}
//			std::cout<<"CSINSTDKF::TDUpdate 3.1"<<std::endl;
			for(pFk=Fk.dd; pFk<pEnd; pFk+=Fk.clm+1)  *pFk+=1.0;
//			std::cout<<"CSINSTDKF::TDUpdate 3.2"<<std::endl;
//			Xk = Fk*Xk;
...
//			std::cout<<"CSINSTDKF::TDUpdate 3.3"<<std::endl;
			for(jj=0; jj<cststt; jj++)  Xk.dd[jj] = Pxz.dd[jj];
			Qk = Qt*tdts;
//			std::cout<<"CSINSTDKF::TDUpdate 3.4"<<std::endl;
//			RtFading(tdts);
...
//				if((iter-2*Ft.row)%2==0)
	...
	measflaglog |= measRes;
//	std::cout<<"CSINSTDKF::TDUpdate return "<<std::endl;
	return measRes;
}
...
{
//	std::cout<<"CSINSGNSS::CSINSGNSS"<<std::endl;
	navStatus = 0;
...
//	std::cout<<"CSINSGNSS::CSINSGNSS return"<<std::endl;
}
...
{
//	std::cout<<"CSINSGNSS::Feedback"<<std::endl;
	CKalman::Feedback(nq, fbts);
//	std::cout<<"CSINSGNSS::Feedback 1"<<std::endl;
//	std::cout<<"avpi.avpinum="<<avpi.avpinum<<std::endl;
	for(int i=0; i<avpi.avpinum; i++) {
		CVect3 tmp(FBXk.dd[3],FBXk.dd[4],FBXk.dd[5]);
		CVect3 tmp1(FBXk.dd[6],FBXk.dd[7],FBXk.dd[8]);
		avpi.vni[i] -= tmp;  
		avpi.posi[i] -= tmp1;
//		avpi.vni[i] -= *(CVect3*)&FBXk.dd[ 3];  avpi.posi[i] -= *(CVect3*)&FBXk.dd[6];
	}
//	std::cout<<"CSINSGNSS::Feedback 2"<<std::endl;
...
//	std::cout<<"CSINSGNSS::Feedback 3"<<std::endl;
	if(nnq==16) {
...
//	std::cout<<"CSINSGNSS::Feedback return "<<std::endl;
}
...
{
//	std::cout<<"CSINS::CSINS()"<<std::endl;
	Init(a2qua(att0), vn0, pos0, tk0);
//	std::cout<<"CSINS::CSINS() return"<<std::endl;
}
...
//	std::cout<<"CSINS::Init"<<std::endl;
	...
	etm(); lever(); Extrap();
//	std::cout<<"CSINS::Init return"<<std::endl;
}
...
//	std::cout<<"DirI() dirIn:"<<dirIn<<std::endl;
...
void CFileRdWt::Init(const char *fname0, int columns0)
{
	...
	strcat(fname, fname0);
//	std::cout<<"CFileRdWt::Init fname: "<<fname<<std::endl;
	...
	else if(columns>0)			// txt file read
	{
		rwf = fopen(fname, "rt");
		if(!rwf) return;
//		std::cout<<"CFileRdWt::Init rwf open "<<std::endl;
//		fpos_t pos;
		long pos;
		/*
...
{
//	std::cout<<"CFileRdWt::load columns="<<columns<<std::endl;
...
//		std::cout<<"buff: "<<buff[0]<<","<<buff[1]<<","<<buff[2]<<std::endl;
	}
	linek += lines;
...
{
//	std::cout<<"sins0.qnb:"<<sins0.qnb.q0<<","<<sins0.qnb.q1<<","<<sins0.qnb.q2<<","<<sins0.qnb.q3<<std::endl;
	Init(sins0);
}
...
{
//	std::cout<<"CAlignkf::Update"<<std::endl;
//	std::cout<<"avpi.avpinum="<<avpi.avpinum<<std::endl;
	...
	qnb = sins.qnb;
//	std::cout<<"CAlignkf::Update return "<<std::endl;
	return res;
}
